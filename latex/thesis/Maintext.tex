...
Based on the plots, we can confirm the increased number of issues at a release, as all projects' most release dates coincide with a peak in issues created. Interestingly, the peaks do not necessarily come after or exactly at the release, and sometimes they preceed the actual release date. Some examples are the \texttt{pandas} version \texttt{v0.24.0} on \textit{January 25, 2019} or version \texttt{v0.25.0} on \textit{July 18, 2019}, or \texttt{py-junos-eznc}'s \texttt{2.3.2} on \textit{April 1, 2020}. Most likely this is an effect of successful testing: before the release, the software is thoroughly tested, and all identified bugs are registered as issues. Since most issues are fixed before the release, users do not find more issues than usual, therefore the issues created drops. 
...
% Plot number of issues with releases --> major, minor or patch has more issues just before the release?

\subsection{Results}
% Metrics:
% - Hierarchy: effective for layoff events
% - Other metics: inconclusive, some are better than others (list them)
% --> these metrics are correlated --> we need to determine which ones depend on which

% Release regularity:
% - major and minor are determinant, patches should be excluded
% - 0.4 CV as identified cutoff value
% - Further examine: we cannot tell whether the release metrics and release regularity are correlated

% Release efficiency, issues:
% TODO ....
