...
In Section \ref{sec:gitminer}, we propose a theoretical framework for analyzing developer social networks (DSNs) in open-source projects, which we also implement using the \texttt{git2net} and \texttt{repo\_tools} mining appls. After discussing the properties of temporal and static networks and describing the relevant network statistics, we implement a Python-based solution for the framework to find an answer to our research question.
